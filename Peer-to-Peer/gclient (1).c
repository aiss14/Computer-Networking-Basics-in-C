...
rnvsH *unmarshall(const unsigned char *head, int fd);
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        if (argc == 6){
            ...
            fprintf(stderr, "usage: hostname, port, type, key\n");
            return 1;
        }
    }
...
//        testData(&req->data->value, &req->data->vLen);
    ...
    /**receive response*/
    unsigned char hBuf[HEADSIZE];    //Buffer for type
    ...
    if (rMsg->data->vLen > 0) {
        fwrite(rMsg->data->value, rMsg->data->vLen, 1, stdout); //add correct printing of 0 bytes / remove termination by 0 bytes
    }
    close(s);
    return 0;
...
unsigned char *marshall(struct rnvsH *res, uint64_t *size) {
    ...
    *size = HEADSIZE + res->data->vLen + res->data->kLen;
    ...
    for (short i = 0; i < res->data->kLen; ++i) {
        msg[i + HEADSIZE] = res->data->key[i];
    }
    for (long i = 0; i < res->data->vLen; ++i) {
